...
const INGREDIENTS_CATALOG = {
    glasses: [
        { id: "classic", name: "圆柱矮杯", price: 0, desc: "经典厚底手绘玻璃矮杯", icon: "🥛" },
        ...
        { id: "milk_carton", name: "牛奶盒杯", price: 250, desc: "萌趣可爱的折角玻璃杯", icon: "🧃" },
    ...
    foams: [
        ...
        { id: "soil_top", name: "深褐花泥", price: 360, emoji: "🪴" },
    ...
    toppers: [
        { id: "none", name: "无顶饰", price: 0, emoji: "🚫" },
        ...
        // 🏪 第三章茶饮特色顶饰 (专属于第三章连锁经营)
        { id: "pecan_nuts", name: "碧根果坚果碎", chapter: 3, price: 0, emoji: "🥜", desc: "香脆喷香的碧根果碎，幽兰拿铁顶饰灵魂！" },
...
const ACHIEVEMENTS_CONFIG = [
    ...
    { id: "star_27", title: "晨曦大师", desc: "关卡中累计集齐 27 颗星星 (第一章全满星)", icon: "🌟", target: 27, type: "stars", rewardDiamonds: 100, rewardCoins: 1000 },
    { id: "star_36", title: "渐变新秀", desc: "关卡中累计集齐 36 颗星星", icon: "🎖️", target: 36, type: "stars", rewardDiamonds: 100, rewardCoins: 1000 },
    { id: "star_45", title: "星芒特调家", desc: "关卡中累计集齐 45 颗星星", icon: "🏅", target: 45, type: "stars", rewardDiamonds: 100, rewardCoins: 1000 },
    { id: "star_54", title: "微醺宗师", desc: "关卡中累计集齐 54 颗星星 (第二章全满星)", icon: "🥇", target: 54, type: "stars", rewardDiamonds: 100, rewardCoins: 1000 },
    { id: "star_63", title: "幻境贤者", desc: "关卡中累计集齐 63 颗星星", icon: "🔮", target: 63, type: "stars", rewardDiamonds: 100, rewardCoins: 1000 },
...
// 本地存档管理器 (全品类支持 + 钻石与成就体系 + 章节与量杯系统)
const StorageManager = {
    KEY: "cozy_bartender_data_v1",
    ...
    getData() {
        ...
        try {
            ...
            if (raw) {
                ...
                if (parsed.coins === undefined) parsed.coins = 100;
    ...
    getAchievementProgress(ach) {
        ...
        } else if (ach.type === "perfectScores") {
            let perfectCount = 0;
            if (data.levelRecords) {
                Object.values(data.levelRecords).forEach(rec => {
                    if (rec && rec.score === 100) perfectCount++;
                });
            }
            current = perfectCount;
        } else if (ach.type === "recipesUnlocked") {
            let unlockedCount = 0;
            if (data.levelRecords) {
                Object.values(data.levelRecords).forEach(rec => {
                    if (rec && rec.score >= 60) unlockedCount++;
                });
            }
            current = unlockedCount;
        } else {
            current = (data.stats && data.stats[ach.type]) || 0;
        }
    ...
    claimAchievement(achId) {
        ...
        if (!ach) return null;
        const prog = this.getAchievementProgress(ach);
        if (!prog.isCompleted || prog.isClaimed) return null;

        const data = this.getData();
        if (!data.claimedAchievements) data.claimedAchievements = [];
        data.claimedAchievements.push(achId);
        data.coins = (data.coins || 0) + ach.rewardCoins;
        data.diamonds = (data.diamonds || 0) + ach.rewardDiamonds;
        this.saveData(data);

        return {
            coins: ach.rewardCoins,
            diamonds: ach.rewardDiamonds,
    ...
    unlockChapter(chapterId, costDiamonds = 300) {
        const data = this.getData();
        ...
        // 若开启第二章且关卡进度未到第10关，自动开启第10关
        if (chapterId === 2 && data.unlockedLevel < 10) {
    ...
    saveLevelResult(level, score, stars) {
        ...
        if (score > prev.score) {
            data.levelRecords[level] = { score, stars };
        }
        if (score >= 60 && level >= data.unlockedLevel && level < DRINK_RECIPES.length) {
